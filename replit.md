...
-   **Idempotent Account Initialization (Jan 2026)**: Added defensive guardrails in `drift-executor.mjs` and `drift-service.ts` to gracefully handle Drift errors 6214 ("Account Already Initialized") and Anchor error 3007 ("AccountOwnedByWrongProgram"). Both errors can occur when RPC returns stale data indicating an account doesn't exist when it actually does. The init transaction may fail with 6214 or 3007 depending on the exact state. Instead of failing the entire operation, the system now logs a warning and proceeds since the account exists (which is the desired outcome). This prevents edge cases from blocking deposits/bot creation.
-   **Batch Account Verification (Jan 2026)**: Fixed critical RPC staleness bug by replacing individual `getAccountInfo` calls with batch `getMultipleAccountsInfo` in `initializeDriftAccountsRaw`. This ensures all account states are checked atomically at the same slot. Additionally, all checks now verify account ownership (`owner === DRIFT_PROGRAM_ID`) rather than just existence, preventing false negatives from corrupted/recycled account data. This fix resolves the issue where wallets with orphaned subaccounts (SA0-SA2) couldn't create new subaccounts (SA3) due to stale RPC returning null for existing accounts.
-   **UserStats numberOfSubAccountsCreated Counter (Jan 2026)**: Critical fix for subaccount ID assignment. Drift Protocol's UserStats account contains two counters:
    -   `numberOfSubAccounts` (offset 184): Current active subaccount count
    -   `numberOfSubAccountsCreated` (offset 186): Total subaccounts ever created (authoritative, never decrements)
    
    The `numberOfSubAccountsCreated` counter only increments and never decrements when subaccounts are closed/deleted. When assigning the next subaccount ID, the system must use `numberOfSubAccountsCreated` as the next ID. The `getNextOnChainSubaccountId()` function now reads both counters from UserStats via `getNumberOfSubAccountsFromUserStats()`, uses `numberOfSubAccountsCreated` as the primary source, and includes cross-validation (totalCreated >= currentCount). The fallback mode restores gap-filling logic for Drift's sequential creation requirement. This fixes the "Invalid sub account id N, must be M" errors (Drift error 6214) that occurred when trying to create subaccounts with IDs lower than the authoritative counter.
