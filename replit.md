...
- **Dynamic Order Scaling (Jan 2026)**: Replaced the rigid collateral gate with intelligent position scaling. When a bot has lost equity, trades are automatically scaled down to fit available margin instead of being rejected. Key improvements: (1) Uses `freeCollateral × leverage` to calculate true max notional capacity, (2) Applies 90% buffer for fees/slippage, (3) Scales trades proportionally when capital is insufficient, (4) Automatically scales back up to original `maxPositionSize` as equity recovers from profitable trades, (5) Only rejects trades if scaled size falls below exchange minimum order size. This ensures continuous trading after losses - the bot adapts its position sizes to available capital rather than stopping entirely.
