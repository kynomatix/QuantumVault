...
-   **Reduce-Only Market Awareness**: 17 Drift markets are marked `reduceOnly` in `MARKET_METADATA_DEFS` (HNT, INJ, RLB, DYM, CLOUD, IO, ZEX, MOTHER, DBR, MICHI, GOAT, FWOG, PNUT, MELANIA, KAITO, IP, 1KWEN). These markets are included in the `/api/drift/markets` response with `isActive: false`, `reduceOnly: true`, and warning text. Bot creation is blocked server-side (`POST /api/trading-bots` rejects with 400). `CreateBotModal` filters them from the market selector. QuantumLab ticker buttons are dimmed with strikethrough and tooltip for reduce-only tickers (via hardcoded `REDUCE_ONLY_TICKERS` set — keep in sync with backend).
