...
### TradingView Signal Processing - CRITICAL FORMULA (Jan 2026)

**DO NOT CHANGE THIS LOGIC. IT IS CORRECT.**

TradingView sends a number in the `contracts` field. The platform interprets this as a PERCENTAGE of the bot's `totalInvestment`.

**The Formula:**
```
tradeAmountUsd = (signal / 100) × botCapital
contractSize = (tradeAmountUsd × leverage) / currentPrice
```

**Example 1 - Single Order:**
- TradingView sends: `contracts: 33.33`
- Bot capital: $1, Leverage: 10x, SOL price: $135
- Step 1: 33.33% × $1 = $0.33 USD
- Step 2: $0.33 × 10x leverage = $3.30 notional
- Step 3: $3.30 ÷ $135 = 0.024 SOL contracts

**Example 2 - Pyramiding Setup:**
- TradingView: Initial capital 100, Order size 25%, Pyramiding 4
- Bot capital: $644
- Each entry: 25% × $644 = $161 per trade
- 4 entries total: $644 (full capital deployed)

**TradingView Settings:**
- User sets "Order size" to a number (e.g., 25 or 33.33)
- User selects "USDT" or "USD" in dropdown (just a label)
- TradingView sends this number in `{{strategy.order.contracts}}`
- Platform interprets the number as a PERCENTAGE

**Why This Works:**
- TradingView's default initial capital is $100
- If user sets order size to "25 USDT", TV sends `contracts: 25`
- Platform reads 25 as 25% of bot capital
- This allows users to scale their real capital independently of TradingView settings
