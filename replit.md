...
-   **QuantumLab (Backtesting)**: Hidden at `/quantumlab` (no nav button). Pine Script strategy backtesting and optimization engine with Gate.io OHLCV data, random search + refinement optimizer, risk analysis, and strategy library. Uses OKX perpetual futures OHLCV data (switched from Gate.io for higher liquidity/quality). Backend at `server/lab/` with APIs at `/api/lab/`. DB tables: `lab_strategies`, `lab_optimization_runs` (with `checkpoint` JSONB column), `lab_optimization_results`, `lab_candle_cache` (persistent OHLCV cache). Max 1 concurrent optimization job to protect core trading operations. Styled to match Docs page (violet/slate theme with sidebar navigation). **Checkpoint/Resume**: Partial results saved directly to `lab_optimization_results` table every 25 random configs (and every 3 refinement seeds), so results persist even if the checkpoint JSONB write fails. Checkpoint JSONB is kept lightweight (iteration counters only, no full results). `saveComboResults` replaces existing partial results for a combo instead of skipping duplicates. `cleanupStaleRuns` checks both checkpoint JSONB and results table — runs with persisted results are marked "paused" not "failed". Initial checkpoint with `configSnapshot` saved immediately on run creation. Resume loads partial results from DB when checkpoint lacks them. Frontend detects dead jobs after 5 failed SSE reconnects and shows appropriate toast. Cancel with progress also pauses instead of failing. **Candle Cache**: OHLCV data cached in PostgreSQL (`lab_candle_cache` table) — survives server restarts unlike the old `/tmp` file cache. First fetch from OKX writes to DB; subsequent requests served from DB instantly. Cache management via `GET /api/lab/cache/stats` and `DELETE /api/lab/cache`. Candle store at `server/lab/candle-store.ts`.
