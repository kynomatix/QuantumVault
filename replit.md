...
-   **Pine Script Compiler**: A Pine-to-JavaScript compiler (`server/lab/pine/compiler.ts`) compiles Pine Script hot loop statements into a single JS function via `new Function()`. Features: dead code elimination (DCE), local variable optimization for non-series decls, `_add` helper to prevent exponential code growth from chained `+` operators (reduced output from 284KB to 47KB — under V8's 55KB TurboFan JIT limit), compiled `for`/`while` loops with step-zero guards and 10K iteration caps matching interpreter safety. The compiled path is automatically used when a `PinePlan` is available (cached on `planRef._compiledCache`), with silent fallback to the interpreter on any compilation or runtime error. Currently at performance parity with the interpreter (~3400ms for Adaptive Regime V3.7, 2316 bars, 127 live stmts). Future optimization paths: expanded series precomputation (84/89 non-var decls are precomputable), worker thread parallelism for optimizer.
