...
- **Reset Drift Account Feature (Jan 2026)**: Added Settings > Danger Zone option to delete all Drift subaccounts and allow recreation with proper referral attribution. Validates using comprehensive account info (USDC balance, total collateral, open positions) and fails explicitly if data cannot be fetched. Deletes subaccounts in reverse order (highest first, 0 last) and clears database assignments.
