...
-   **Pine Script User Functions & Dynamic TA**: The interpreter supports user-defined functions (e.g., `f_hma`, `f_adx` in Flux Momentum) with incremental TA computation for dynamic sources. All TA functions (SMA, EMA, RMA, WMA, RSI, stdev, highest, lowest) have dedicated `incremental*` functions for per-bar computation inside user functions. The `computeOnFly` function uses lazy array allocation (h/l/cl arrays only created when needed, not on every call) to avoid O(n) overhead on cache hits.
