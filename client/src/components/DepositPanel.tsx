...
interface DepositPanelProps {
  /** Whether the panel is currently visible. Drives token loading and state reset. */
  active: boolean;
  usdcBalance: number | null;
  /** Called after a successful deposit (USDC or token→swap). */
  onComplete: () => void;
  /** Optional: called when the panel should close itself (after a successful deposit). */
  onClose?: () => void;
  /** Optional: notifies the parent whether a deposit/swap is in flight (to block closing). */
...
/**
 * Reusable deposit UI: a USDC tab (direct deposit) and an "Any asset" tab
 * (deposit any wallet token, server-swapped to USDC). Has no Dialog wrapper so
 * it can be embedded inside any dialog/onboarding flow.
 */
export function DepositPanel({
  ...
  useEffect(() => {
    if (active) {
      ...
      // Reset transient state when hidden.
      setUsdcAmount('');
      setTokenAmount('');
      setSelected(null);
      setQuote(null);
  ...
  useEffect(() => {
    onProcessingChange?.(isDepositingUsdc || isProcessing);
  }, [isDepositingUsdc, isProcessing, onProcessingChange]);
  ...
  useEffect(() => {
    if (active && tab === 'token' && tokens.length === 0 && !tokensLoading) {
      fetchTokens();
    }
  }, [active, tab, tokens.length, tokensLoading, fetchTokens]);
