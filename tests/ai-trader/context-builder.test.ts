...
describe("buildMarketContext (WO-3)", () => {
  ...
  it("claimed-from-idle paper bot uses pre-claim authority and resolves zero rows flat", async () => {
    const { buildMarketContext } = await import("../../server/ai-trader/context-builder");
    const result = await buildMarketContext({
      market: "SOL-PERP",
      timeframe: "15m",
  ...
  it("genuinely pre-existing analyzing paper state remains unknown with zero rows", async () => {
    const { buildMarketContext } = await import("../../server/ai-trader/context-builder");
    const result = await buildMarketContext({
      market: "SOL-PERP",
      timeframe: "15m",
    ...
    expect((result.contextDigest as any).account).toMatchObject({
      ...
      positionReason: "status_requires_open_row",
      hasPosition: null,
    });
