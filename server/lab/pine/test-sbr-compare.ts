...
async function main() {
  ...
  const result = runSbrBacktest(candles, sbrParams, "SOL/USDT:USDT", "2h", config);
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    if (tv && our) {
      ...
      if (match) matchCount++;
      console.log(
        `${(i + 1).toString().padStart(3)} ${our.direction.padEnd(6)} ` +
        `${tv.entry.toFixed(2).padStart(10)} ${our.entryPrice.toFixed(2).padStart(10)} ` +
        `${tv.exit.toFixed(2).padStart(10)} ${our.exitPrice.toFixed(4).padStart(10)} ` +
        `${tv.pnl.toFixed(2).padStart(10)} ${our.pnlPercent.toFixed(2).padStart(10)} ` +
        `${(match ? "✓" : "✗").padStart(7)}`,
      );
    } else if (tv) {
      console.log(
        `${(i + 1).toString().padStart(3)} ${tv.dir.padEnd(6)} ` +
        `${tv.entry.toFixed(2).padStart(10)} ${"MISSING".padStart(10)} ` +
        `${tv.exit.toFixed(2).padStart(10)} ${"MISSING".padStart(10)} ` +
        `${tv.pnl.toFixed(2).padStart(10)} ${"N/A".padStart(10)} ${"✗".padStart(7)}`,
      );
    } else if (our) {
      console.log(
        `${(i + 1).toString().padStart(3)} ${our.direction.padEnd(6)} ` +
        `${"EXTRA".padStart(10)} ${our.entryPrice.toFixed(2).padStart(10)} ` +
        `${"EXTRA".padStart(10)} ${our.exitPrice.toFixed(4).padStart(10)} ` +
        `${"N/A".padStart(10)} ${our.pnlPercent.toFixed(2).padStart(10)} ${"✗".padStart(7)}`,
      );
    }
  }
  ...
  if (result.trades.length > tvTrades.length + 3) {
    console.log(`\n⚠  Extra trades detected — first few extras:`);
    for (let i = tvTrades.length; i < Math.min(result.trades.length, tvTrades.length + 5); i++) {
      const t = result.trades[i];
      console.log(`  EXTRA T${i + 1}: ${t.direction} entry=${t.entryPrice.toFixed(2)} exit=${t.exitPrice.toFixed(2)} pnl=${t.pnlPercent.toFixed(2)}% reason=${t.exitReason} time=${t.entryTime}`);
    }
  }
}
