...
async function getExchangeBalance(walletAddress: string, subAccountId: number = 0): Promise<number> {
  try {
...
async function getAgentExchangeBalance(agentPublicKey: string): Promise<number> {
  return getExchangeBalance(agentPublicKey, 0);
}

async function getExchangeAccountInfo(walletAddress: string, subAccountId: number = 0) {
  try {
    const info = await getDefaultAdapter().getAccountInfo(walletAddress, _subIdStr(subAccountId));
...
async function getExchangeAccountInfoForBot(agentPublicKey: string, subAccountId: number, botCtx: BotSubaccountContext | null) {
  if (botCtx) {
    try {
  ...
  return getExchangeAccountInfo(agentPublicKey, subAccountId);
}
...
async function computeTradeSizingAndTopUp(params: TradeSizingParams): Promise<TradeSizingResult> {
  ...
  try {
    const accountInfo = await getExchangeAccountInfoForBot(agentPublicKey, subAccountId, botCtx ?? null);
...
`;
    ...
    try {
      ...
      for (const bot of bots) {
        ...
        try {
          ...
          if (capBotCtx) {
            const liveInfo = await getExchangeAccountInfoForBot('', 0, capBotCtx);
  ...
  // One-off recovery for orphaned Pacifica subaccounts. When the V3
  // bot-subaccount writer failed after provisionFundedSubaccount had already
  // transferred funds into a freshly-created subaccount, the sub keypair was
  // zeroized on rollback — leaving funds stranded in a subaccount whose key
  // we no longer hold. This endpoint attempts a main-signed sub→main
  // transfer via Pacifica's /account/subaccount/transfer endpoint. Pacifica
  // may reject it (some venues require the sub's own signature even for
  // owned subs); if so we'll need their support to release the funds.
  app.post("/api/exchange/recover-orphan-subaccount", requireWallet, async (req, res) => {
    try {
      const { fromSubaccount, amount } = req.body as { fromSubaccount?: string; amount?: number };
      if (!fromSubaccount || typeof fromSubaccount !== 'string') {
        return res.status(400).json({ error: "fromSubaccount (string) required" });
      }
      ...
      try {
        ...
        agentKeyResult = await decryptAgentKeyStrict(
          req.walletAddress!,
          umkResult.umk,
          walletForDecrypt,
          walletForDecrypt.agentPublicKey,
      ...
      try {
        ...
        if (adapter.protocolName !== 'pacifica') {
          return res.status(400).json({ error: `Recovery only implemented for pacifica, active=${adapter.protocolName}` });
        }
        const pacificaAdapter = adapter as import('./protocol/pacifica/pacifica-adapter').PacificaAdapter;

        console.log(`[OrphanRecovery] wallet=${req.walletAddress!.slice(0,8)}... from=${fromSubaccount.slice(0,8)}... to=${wallet.agentPublicKey.slice(0,8)}... amount=$${amount}`);

        const result = await pacificaAdapter.transferBetweenSubaccounts({
        ...
        console.log(`[OrphanRecovery] result success=${result.success} error=${result.error || 'none'}`);

        if (!result.success) {
          return res.status(502).json({
            success: false,
            error: result.error || 'Pacifica rejected the transfer',
            note: "Pacifica may require the sub's own signature. Contact Pacifica support with proof of ownership from your main agent account.",
          });
        }
        return res.json({ success: true, fromSubaccount, toMain: wallet.agentPublicKey, amount });
      } finally {
        agentKeyResult.cleanup();
      }
    } catch (err: any) {
      console.error('[OrphanRecovery] error:', err?.message || err);
  ...
  app.get("/api/health-metrics", requireWallet, async (req, res) => {
    try {
      ...
      for (const bot of bots) {
        ...
        if (hmBotCtx) {
          try {
            const botInfo = await getExchangeAccountInfoForBot('', 0, hmBotCtx);
  ...
  app.get("/api/bots/:botId/balance", requireWallet, async (req, res) => {
    try {
      ...
      if (balBotCtx) {
        try {
          const liveInfo = await getExchangeAccountInfoForBot(wallet.agentPublicKey!, subAccountId, balBotCtx);
      ...
      const enrichedBots = await Promise.all(bots.map(async (bot) => {
        ...
        try {
          ...
          if (botCtx && wallet?.agentPublicKey) {
            const liveInfo = await getExchangeAccountInfoForBot(wallet.agentPublicKey, 0, botCtx);
            botBalance = liveInfo.totalCollateral;
  ...
  app.delete("/api/trading-bots/:id", requireWallet, async (req, res) => {
    ...
    try {
      ...
      const mainBalance = await getExchangeBalance(agentAddress, 0);
      if (mainBalance > 0.01) {
              ...
              if (autoWithdrawThreshold > 0) {
                try {
                  ...
                  const accountInfo = botCtx
                    ? await getExchangeAccountInfoForBot(wallet.agentPublicKey!, subAccountId, botCtx)
                    : await getExchangeAccountInfo(wallet.agentPublicKey!, subAccountId);
    ...
    try {
      ...
      for (const bot of bots) {
        ...
        try {
          ...
          if (eqBotCtx) {
            const liveInfo = await getExchangeAccountInfoForBot('', 0, eqBotCtx);
            botBalance = liveInfo.totalCollateral;
          } else {
            let prices: Record<string, number> = {};
  ...
  app.post("/api/trades/:tradeId/retry", requireWallet, async (req, res) => {
    try {
      ...
      if (bot.autoTopUp && targetEquity > 0) {
        try {
          const accountInfo = await getExchangeAccountInfoForBot(wallet.agentPublicKey!, subAccountId, retryBotCtxForTopUp);
